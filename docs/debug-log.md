...
## 4. 新增困难样本与重复图片（2026-08-29）

现象：补充了 8 张桌面场景照片，其中包含机器人泡沫包装、显示器、键盘和
鼠标等干扰/遮挡场景；其中两张图片二进制内容相同。

处理：按 SHA256 去重，重复图片不重复入库。画面中能看到鼠标或水杯的图片不
自动设为空标签，而是写入 `annotation_required`，在 X-AnyLabeling 中人工补框；
确认不含目标的显示器场景写入 `human_checked_negative` 并保留空 JSON。待人工
审核完成前，导出脚本会将这些图片列入 `excluded_pending.csv`，不会混入训练集。
